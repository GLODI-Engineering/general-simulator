...
- `example.cir` — the doc comment's own `## Example`: a MOSFET held permanently on via
  `gate=block ctrl=<sig2voltage>`. Checked that `V(out)` settles to
  `5 * 1000 / (1000 + 0.1)` (a fully-on 0.1 Ω MOSFET in series with a 1 kΩ load).
