use crate::IdealDiode;

/// An idealized power switch modeling a real MOSFET's behavior: a controlled switch when gated
/// on, falling back to its intrinsic body diode when gated off.
///
/// Named `IdealSwitch` rather than `Mosfet` because this is a PWL companion model (`Ron` when
/// gated on, a PWL body diode when gated off) -- not the real BSIM-style device physics
/// ("MOSFET" is reserved for a future, not-yet-implemented model of that kind). See
/// `docs/journal/` for the rename that introduced this name.
///
...
/// see `dae-runtime`'s `solve_dc_with_ideal_switches`.
///
...
/// anode - cathode > v_th`), with the physical anode at the switch's own source and cathode at
...
/// [`IdealSwitch::body_diode_for_drain_source_stamping`], which every caller stamping this
/// device's off-state should use instead of `body_diode` directly. (An earlier version of this
/// struct required the netlist to declare `(source, drain)` instead, so the *unmirrored*
/// `body_diode` stamped correctly as-is — that convention was a real, confirmed footgun: 6 of 8
/// switches in the TIDA-010954 cycloconverter experiment were declared in the natural,
/// SPICE-conventional `(drain, source)` order and so had their body diodes backwards. This
/// struct's contract changed specifically to remove that footgun, not just document it better.)
#[derive(Debug, Clone, Copy, PartialEq)]
pub struct IdealSwitch {
    ...
    /// [`IdealSwitch::body_diode_for_drain_source_stamping`] instead, which returns the curve
    /// already mirrored for that node order.
    pub body_diode: IdealDiode,
}

impl IdealSwitch {
    pub fn new(r_on: f64, body_diode: IdealDiode) -> Self {
        IdealSwitch { r_on, body_diode }
    }

    /// The body diode's curve, mirrored so that evaluating it with `V = V(drain) - V(source)`
    /// (the plain SPICE-conventional node order this crate's netlists declare this device's two
    /// terminals in) gives the physically correct result: blocks for `V > 0`, conducts for `V
    /// < -v_th`. See [`IdealDiode::reversed`] for the derivation, and this struct's own doc
    /// comment for why this indirection exists at all instead of just documenting a required
    /// node order.
    pub fn body_diode_for_drain_source_stamping(&self) -> IdealDiode {
...
mod tests {
    ...
    fn body_diode_blocks_below_threshold_and_conducts_above() {
        let m = IdealSwitch::new(0.05, IdealDiode::new(0.0, -1e4, 0.0, 0.7, 1.0));
    ...
    fn body_diode_for_drain_source_stamping_matches_plain_spice_node_order() {
        let m = IdealSwitch::new(0.05, IdealDiode::new(0.0, -1e6, 1e-6, 0.6, 100.0));
