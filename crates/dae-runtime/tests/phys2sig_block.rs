//! `BlockKind::Phys2Sig` — the *only* way a circuit quantity enters the signal domain (the
//! enforced physical/signal-domain boundary; see that type's own doc comment). Checks both
//! `Phys2SigTarget::Voltage` (`V(node)`) and `Phys2SigTarget::Current` (`I(branch)`) against a simple
...
//! the phys2sig block's own read path, not a fresh derivation of the RC response.

use std::collections::BTreeMap;

use dae_runtime::{
    simulate_transient_with_blocks, BlockInstance, BlockKind, Phys2SigTarget, TimeStep,
};
use general_spice_core::Dialect;
use pwl_devices::{IdealDiode, IdealSwitch};

#[test]
fn phys2sig_voltage_and_current_match_hand_derived_rc_charging_curve() {
    ...
    // A phys2sig block reads the circuit's *previous* step (the same sampled-data convention
    // every block-graph read of circuit state uses) -- step i's own output must equal step
    // (i-1)'s resolved V(vout) exactly (step 0 reads the pre-run all-zero initial point, i.e.
    // 0.0), and only *then* is that value close to the analytic curve evaluated one dt earlier
    // than the step's own timestamp.
    ...
    for (t, point, outputs) in &trace {
        ...
        assert!(
            ...
            "t={t}: VOUT_PROBE={probed_vout} disagrees with the previous step's own \
             V(vout)={prev_vout}"
        );
    ...
    assert!(
        max_v_err < 0.05,
        "V(vout) deviates from the hand-derived RC charging curve (evaluated one dt earlier, \
         matching the phys2sig block's own previous-step read) by {max_v_err}"
    );

    // I(V1) phys2sig: current through the source into a charging capacitor should start near
    // decay toward 0. Step 0 reads the pre-run all-zero initial point (current genuinely 0
    // there, not yet meaningful); step 1 already reads a solved point with substantial charging
    // current, so that's the right index for the "not near zero yet" check.
