...
- A block-driven independent voltage/current source in the circuit itself (today, data flows
  block-to-circuit only through a MOSFET gate; a block cannot inject current back into the MNA
  system) — noted as a real limitation in at least one worked experiment
  (`elspice-pwl-buck-dc-motor-cascade`).
