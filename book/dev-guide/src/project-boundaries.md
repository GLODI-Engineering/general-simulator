...
## What goes here
- The sibling-repo map: `spice-lsp`/`spice-core` (sole netlist-parsing authority),
  `elspice-mna` (sole linear-device MNA stamping + numeric Schur-complement reduction, plus
...
- The explicit escalation rule: a change to a sibling repo needs the user's confirmation before
  ...
  precedent (`elspice-mna` gained `TransientFunction`/`SIN`/`PWL` support in exactly this way,
...
- `elspice-pwl`'s journal entry documenting the stale-docs correction (SIN/PWL support) as the
