...
- Restate, briefly (link back rather than re-deriving): diode segment choice is resolved by the
  ...
  *fixed* topology.
- A gated-off ideal switch's body diode is folded into the exact same diode set, via
  `IdealSwitch::body_diode_for_drain_source_stamping` (not `body_diode` directly) — explain the
...
- `crates/pwl-devices/src/ideal_switch.rs` module doc comment in full — the physics explanation
  and the node-order rationale (including the "earlier version of this struct had it backwards,
  a real confirmed footgun" account) are already written at exactly this depth.
- `crates/dae-runtime/src/lib.rs`'s `build_with_ideal_switches` function.
